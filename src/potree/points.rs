...
pub struct LoadPointsTaskHolder {
    ...
    pub loaded_points: HashSet<String>,
...
pub fn load_points(
    ...
    for (entity, mut load_points_task_holder, hierarchy_snapshot, transform) in
        potree_point_clouds_3d
    {
        for octree_node in &hierarchy_snapshot.0 {
            if !load_points_task_holder
                .loaded_points
                .contains(&octree_node.name)
            {
                if let Ok(_) =
                        ...
                        })
                {
                    load_points_task_holder
                        .loaded_points
                        .insert(octree_node.name.clone());
                }
            }
        }

        while let Ok(loaded_points) = load_points_task_holder.loaded_points_rx.try_recv() {
            let point_cloud_handle = point_clouds.add(loaded_points.point_cloud);
